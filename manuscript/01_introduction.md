...
- **A primitive library spanning {{DOMAIN_COUNT}} domains** —
  {{DOMAIN_BULLETS}}
...
{{DOMAIN_COUNT}} primitive domains:
{{DOMAIN_BULLETS}}

The grammar seed `{{GRAMMAR_SEED}}` is the single source of randomness for
every selection made during expansion; the grammar hash `{{GRAMMAR_HASH}}`
