...
effective figure. Both `{{PRODUCT_SIZE}}` (nominal) and
`{{EFFECTIVE_PRODUCT_SIZE}}` (effective) are reported in this manuscript
...
`{{RESERVED_SLOT_COUNT}}` reserved slots (`{{RESERVED_SLOT_NAMES}}`) are
...
first {{HASH_PREFIX_HEX_LENGTH}} hex characters of its SHA-256 — the same truncation convention used
for `grammar_hash`.
...
produce a new seed masked to {{DERIVED_SEED_BITS}} bits (keeping it a
