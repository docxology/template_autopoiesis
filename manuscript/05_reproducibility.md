...
uv run python scripts/autopoiesis.py expand --seed {{GRAMMAR_SEED}} --output output/spec.json

# 2. Materialize the spec into a runnable child project + provenance.json
uv run python scripts/autopoiesis.py materialize --seed {{GRAMMAR_SEED}} --out-root output/children
...
so the reported {{COVERAGE_PCT}} agrees with the authoritative CI gate
methodology rather than silently using a different one) to produce the
{{TEST_COUNT}} and {{COVERAGE_PCT}} tokens substituted above — the same
