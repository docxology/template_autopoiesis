...
\begin{block}{Why generate projects at all}
\protect\phantomsection\label{why-generate-projects-at-all}
...
\begin{block}{The word ``autopoietic'' is doing real work, not
decoration}
\protect\phantomsection\label{the-word-autopoietic-is-doing-real-work-not-decoration}
...
\breakseq{[@maturana\_varela\_1980]}. That is a strong claim about biology, and
...
\begin{block}{Three claims that are usually asserted, rarely checked}
\protect\phantomsection\label{three-claims-that-are-usually-asserted-rarely-checked}
...
\breakseq{[@reproducible\_builds]}. The project treats ``trust the README'' as
a failure mode to be engineered around, not a baseline to build on.
\end{block}

\begin{block}{What problem seeded, deterministic generation actually
solves}
\protect\phantomsection\label{what-problem-seeded-deterministic-generation-actually-solves}
...
trusting a single example \breakseq{[@claessen2000quickcheck;
@maciver2019hypothesis]}). A generator whose output depends on
wall-clock time, dict iteration order, or an unseeded RNG cannot make
this promise, and would fail the very first time someone tried to hold
it to its own claim.
...
\breakseq{[@merkle\_tree\_provenance]}.
\end{block}

\begin{block}{Contributions of this exemplar}
\protect\phantomsection\label{contributions-of-this-exemplar}
