...
![Total, effective, and reserved-only product size for the grammar at seed={{GRAMMAR_SEED}}, generated by `fig_product_space_annotation` (`src/manuscript_figures.py`) directly from `Grammar.product_size` / `Grammar.effective_product_size` — not hand-typed.](../output/figures/fig_product_space.png){#fig:product_space width=80% fig-alt="Bar chart comparing total, effective, and reserved-only grammar product space."}

Every row in the table above is a `GrammarSlot` parsed from `docs/manuscript/config.yaml`
...
and takes the first sixteen hex characters of its SHA-256 digest — `{{GRAMMAR_HASH}}`
...
than a combinatorial trick that multiplies unrelated toggles: {{DOMAIN_COUNT}}
